...
class PaymentController extends Controller
{
    ...
    public function redirectHandler(Request $request)
    {
        ...
        return redirect()->away(
            "{$frontendUrl}/payments/confirm?reference={$reference}&delivery_id={$deliveryId}"
        );
    }
    ...
    private function handleSuccessfulPayment(Delivery $delivery, ?Payment $payment, string $reference): void
    {
        if (!$delivery->tracking_number) {
            ...
            if ($driver) {
                ...
                $delivery->status = DeliveryStatusEnums::QUEUED;
                $delivery->driver_assigned_at = now();
                $delivery->save();
    ...
    {
        if ($payment) {
            $payment->status = PaymentStatusEnums::FAILED;
            $payment->reference = $reference;
            $payment->save();
        }
