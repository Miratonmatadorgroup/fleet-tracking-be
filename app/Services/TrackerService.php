...
class TrackerService
{
    ...
    public function __construct()
    {
        $this->baseUrl = config('services.tracker.tracker_node_base_url');
    }

    // public function getLastPosition(array $deviceIds)
    // {
    //     return Http::post(
    //         "{$this->baseUrl}/devices/last-position",
    //         [
    //             'imeis' => $deviceIds
    //         ]
    //     )->json();
    ...
    public function lockVehicle(string $imei)
    {
        return Http::post(
            "{$this->baseUrl}/devices/{$imei}/commands/shutdown"
        )->json();
    }

    public function unlockVehicle(string $imei)
    {
        return Http::post(
            "{$this->baseUrl}/devices/{$imei}/commands/unlock"
        )->json();
    }
    ...
    public function getMileageDetail(
        ...
        return Http::get(
            "{$this->baseUrl}/devices/{$imei}/mileage",
            [
                'start' => $start,
                'end' => $end
            ]
